...
if test $with_edgex = true; then
  ...
  echo -e "---------------------------------------------\n"
   ...
   /opt/jmeter/bin/jmeter.sh -Jjmeter.save.saveservice.output_format=xml -n -t test/edgex_sink_rule2.jmx -Dfvt="$fvt_dir" -l jmeter_logs/edgex_sink_rule.jtl -j jmeter_logs/edgex_sink_rule.log
     echo -e "---------------------------------------------\n"
